...
def degrees_for_ranges(comp_table, low_distance, high_distance):
    low_degrees = comp_table[low_distance]
    high_degrees = comp_table[high_distance]
    return low_degrees, high_degrees


def compensation(comp_table, current_distance):
    low_key_distance, high_key_distance = closest_keys_to_current_distance(comp_table.keys(), current_distance)
    low_value_degrees, high_value_degrees = degrees_for_ranges(comp_table, low_key_distance, high_key_distance)
    delta_degrees = high_value_degrees - low_value_degrees
    if high_key_distance - low_key_distance != 0:
        percent_current_distance_between_low_distance_and_high_distance = (current_distance - low_key_distance) / (
                    high_key_distance - low_key_distance)
    else:
...
def test_compensation(distance_in_m, desired_angle_in_degrees):
    ...
    logging.debug(
        f"Given {distance_in_m}, I think compensation is {compensation_degrees} while \
        desired is {desired_angle_in_degrees}")
    assert compensation_degrees == desired_angle_in_degrees


if __name__ == '__main__':
    test_compensation(1, 0)
    test_compensation(6, 4)
    test_compensation(3.5, 2)
    test_compensation(10, 12)
