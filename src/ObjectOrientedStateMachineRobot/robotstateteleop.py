...
class RobotStateTeleop(RobotState):
    ...
    def periodic(self, robot):
        ...
        self.intake.get_teleop_periodic_commands(self.wrist_position, self.intake_control)

        botpose = self.robot.vision.check_botpose()
        ...
        self.shooter.get_teleop_periodic_commands(shooter_pivot_control, shooter_control, kicker_action)
    ...
    def do_outtake(self):
        self.wrist_position = WristAngleCommandEnum.wrist_stow_action
        self.intake_control = IntakeCommandEnum.outtake_action
    ...
    def pivot_control_periodic(self, controller_state : ControllersState):
        if controller_state.left_stick_button:
            shooter_pivot_control = self.shooter_pivot_manual_up
        elif controller_state.right_stick_button:
            shooter_pivot_control = self.shooter_pivot_manual_down
        else:
            shooter_pivot_control = 0
        return PivotControlCommand(shooter_pivot_control)
