...
from ShooterWidget import ShooterWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, nt_server_ip : str = 'localhost'):
        ...
        layout.addWidget(ShooterWidget())
    ...
    def _build_control_panel(self):
        ...
        self.shooter_angle_label = QLabel("Shooter Angle: 0")
        control_panel_layout.addWidget(self.shooter_angle_label)

        self.shooter_widget = ShooterWidget()
        control_panel_layout.addWidget(self.shooter_widget)
        self.shooter_widget.setFixedSize(200, 200)
    ...
    def update_robot_pos(self):
        ...
        speaker_x = 7.8
        speaker_y = 1.4
        speaker_z = 0.1 # Not used yet, but likely important for shooter-angle calculations

        robot_x = self.robot.field_pos[0]
        robot_y = self.robot.field_pos[1]
        robot_z = 0.1 # Hard code for now
        ...
        calculated_shooter_angle = self.calc_shooter_angle(distance_to_speaker, speaker_z, robot_z)
        self.shooter_angle_label.setText(f"Shooter Angle: {calculated_shooter_angle:.1f}°")
        self.shooter_widget.set_shooter_angle(calculated_shooter_angle)
    ...
    def calc_shooter_angle(self, distance_to_speaker, speaker_z, robot_z):
        """
        Calculate the angle the shooter should be at to hit the speaker
        """
        # Calculate the height difference between the robot and the speaker
        height_difference = speaker_z - robot_z

        # Calculate the angle to the speaker
        angle_radians = math.atan2(height_difference, distance_to_speaker)
        angle_degrees = math.degrees(angle_radians)
        return angle_degrees

    def calc_distance_to_speaker3D(self, speaker_x, speaker_y, speaker_z, robot_x, robot_y, robot_z):
        """
        Calculate the distance to the speaker
        """
        # Calculate the distance to the speaker
        # Basic Pythagorean theorem
        distance_to_speaker = ((speaker_x - robot_x) ** 2 + (speaker_y - robot_y) ** 2 + (speaker_z - robot_z) ** 2) ** 0.5
        return distance_to_speaker
